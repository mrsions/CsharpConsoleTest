...
class BlowfishCipher
{
    ...
    {
        return MakeRandomBytes(8);
    }
    public static byte[] MakeRandomKey()
    {
        return MakeRandomBytes(56);
    }
    ...
    public byte[] IV
    {
        ...
        set
        {
            if (value.Length == 8)
            {
                Buffer.BlockCopy(value, 0, iv, 0, 8);
                IVSet = true;
            }
            else
            {
    ...
    public BlowfishCipher(byte[] cipherKey)
    {
        SetupKey(cipherKey);
    }
    ...
    public byte[] Encrypt(byte[] src, int offset = 0, int length = -1)
    {
        if (length == -1) length = src.Length;
        int dstLen = length % 8 == 0 ? length : length + 8 - (length % 8);
        byte[] result = new byte[dstLen];
        Encrypt(src, offset, length, ref result, out _);
        return result;
    }
    public void Encrypt(byte[] src, int offset, int length, ref byte[] dst, out int dstLen)
    {
        if (!IVSet)
        {
        ...
        //dstLen = length % 8 == 0 ? length : length + 8 - (length % 8);
        //if (dst == null || dst.Length < dstLen) dst = new byte[dstLen];
    ...
    public byte[] Decrypt(byte[] src, int offset = 0, int length = -1)
    {
        if (length == -1) length = src.Length;
        int dstLen = length % 8 == 0 ? length : length + 8 - (length % 8);
        byte[] result = new byte[dstLen];
        Decrypt(src, offset, length, ref result, out _);
        return result;
    }
    public void Decrypt(byte[] src, int offset, int length, ref byte[] dst, out int dstLen)
    {
        if (!IVSet)
        {
            throw new Exception("IV not set.");
    ...
    private void CopyToIV(byte[] src, int offset)
    {
        // custom
        //uint crc = Crc32.Compute(iv);
        //for (int i = 0; i < 8; i++)
        //{
        //    iv[i] = (byte)((iv[i] ^ src[offset + i]) & (iv[i] ^ (crc >> i )) & 0xFF);
        //}
        // original
        for (int i = 0; i < 8; i++)
        {
    ...
    private void SetBlock(byte[] block, int i)
    {
        xl_par = (((uint)block[i + 3] << 0x00) & 0xFF)
               | (((uint)block[i + 2] << 0x08) & 0xFF00)
               | (((uint)block[i + 1] << 0x10) & 0xFF0000)
               | (((uint)block[i + 0] << 0x18) & unchecked(0xFF000000));

        xr_par = (((uint)block[i + 7] << 0x00) & 0xFF)
               | (((uint)block[i + 6] << 0x08) & 0xFF00)
               | (((uint)block[i + 5] << 0x10) & 0xFF0000)
               | (((uint)block[i + 4] << 0x18) & unchecked(0xFF000000));
    }
    ...
    private void GetBlock(byte[] block, int i)
    {
        block[i + 3] = (byte)((xl_par >> 0x00) & 0xFF);
        block[i + 2] = (byte)((xl_par >> 0x08) & 0xFF);
        block[i + 1] = (byte)((xl_par >> 0x10) & 0xFF);
        block[i + 0] = (byte)((xl_par >> 0x18) & 0xFF);

        block[i + 7] = (byte)((xr_par >> 0x00) & 0xFF);
        block[i + 6] = (byte)((xr_par >> 0x08) & 0xFF);
        block[i + 5] = (byte)((xr_par >> 0x10) & 0xFF);
        block[i + 4] = (byte)((xr_par >> 0x18) & 0xFF);
    }
